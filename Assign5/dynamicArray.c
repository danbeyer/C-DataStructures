 ...
 * Name: Daniel Beyer
 * Date:  07/27/16
 * This is an implementation of heap-based Priority Queue and implementation of the
 * Heapsort algorithm
 */
...
{
    int lft = (position * 2) + 1;
    int rght = (position *2) + 2;
    int smallest;

    if(rght<last) {
        if(compare(dyGet(heap, lft), dyGet(heap, rght)) == -1) {
            smallest = lft;
        }
        else {
            smallest = rght;
        ...
        if(compare(dyGet(heap, lft), dyGet(heap, position)) == -1) {
            dySwap(heap, position, lft);
            adjustHeap(heap, last, lft, compare);
        }
    }
}
